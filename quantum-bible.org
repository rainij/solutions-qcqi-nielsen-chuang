...
*Solution:* We divide the proof into several steps.
...
*E:* $R_{\hat{n}}(\alpha)$ corresponds to a rotation of angle $\alpha$ around /some/ axis (which is
independent of the angle).

PROOF: This is a direct consequence of (D), together with (B) and (C). QED.

*F:* The angle in (E) is indeed $\hat{n}$, which concludes the solution of exercise 4.6.
