...
/this/ context. We do not need any clever idea here. In fact, the basic idea is this:

<<exercise-5-24-s>>
$$
  s \approx \tilde{s} = \frac{\widetilde{sl_2/r}}{\widetilde{l_2/r}} ,
$$

where the tilde stands for the fact that we merely have an approximation of the underlying
value available. It remains to formulate conditions under which this approximation is
close enough to $s$ so that rounding to the nearest integer yields the correct value for $s$.

Let us discuss where the error comes in. First of all let us note that the $\approx$ in
step 3 of the algorithm is actually an equality ($=$). In fact, this is proved by
exercises [[#exercise-5-22][5.22]] and [[#exercise-5-23][5.23]]. The error actually happens when doing the inverse Fourier
transform in step 4. In section 5.2.1 it was shown that with probability of at least
$1-\varepsilon$ the two approximations are exact up the $t$​th bit. To keep things simple we
assume that the quotient on the RHS of the [[exercise-5-24-s][approximation]] of $s$ can be calculated
exactly. This is actually possible using rational arithmetic (use =fractions.Fraction= in
python).

Let $\varphi_1=sl_2/r$, $\varphi_2=l_2/r$ and define

\begin{align*}
  \varphi_1 + \delta_1 &:= \tilde{\varphi_1}, \\
  \varphi_2 + \delta_2 &:= \tilde{\varphi_2} .
\end{align*}
...
The conditions on $s$ and $r$ are justified since $s=0$ means $b=1$ and trying all $s\,<\,r$
is also doable classically if $r$ is small. The case $l_2=0$ has a low probability; in
fact, all $r$ possible values for $l_2$ have equal probability by
$\norm{\ket{\hat{f}(sl_2,l_2)}}=\sqrt{r}$ which in turn can by seen from [[exercise-5-22-to-show][the formula]] for
$\ket{\hat{f}(l_1,l_2)}$ in exercise 5.22.

By chapter 5.2.1 we have $\delta_i\leq2^{-t}$. Moreover let us choose $t=\log_2(r^c)$ for
some yet to determine constant $c$ (note that this satisfies $t=O(\log(r))$ consistent
with the runtime requirements). Hence

$$
  \abs{\delta_i} \leq 2^{-t} = r^{-c} .
$$

Using this and the [[reasonable-assumptions][reasonable assumptions]] we deduce

$$
  \delta := \max{\frac{\abs{\delta_1}}{\varphi_i}} \leq 2 r^{1-c} .
$$

if we *assume* $c\geq2$, because then $\delta_i\leq1/2r\leq\varphi_i/2$. Using this we get:

$$
  \abs{s - \tilde{s}} = s \abs{1 - (1+\delta_1/\varphi_1)(1+\delta_2/\varphi_2)\inv}
  \leq s((1 - \delta)^{-2} - 1)
  \leq 4r \delta
  \leq 8r^{2-c} .
$$

Here we also used that $(1-\delta)^{-2}\leq1+4\delta$ which is true if (for example)
$\delta\leq1/4$ which in turn holds for $r\geq8$ if $c\geq2$.

We want that $\abs{s-\tilde{s}}\leq1/4$. This is satisfied if

$$
  8 r^{2-c} \leq \frac{1}{4} ,
$$

Which in turn is equivalent to

$$
  c \geq 2 + \frac{5}{\log_2(r)} .
$$

Let us just take $c$ equal to the RHS. We have just showed that then the [[exercise-5-24-s][approximation]] for
$s$ is good enough.

*Summary:* Choosing $t=2\ceil{\log_2(r)}+5$ is sufficient for the algorithm to work with high
probability under [[reasonable-assumptions][reasonable assumptions]].
