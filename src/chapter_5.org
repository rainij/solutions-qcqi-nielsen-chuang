...
#+begin_src python
  def make_qft_circuit(size: Integral) -> QuantumCircuit:
...
#+RESULTS[34c89f955a4b13ceb93a35102b0e35d2af6d11c4]:
:                                                                           ┌───┐
: q_0: ──────■───────────────────────────────■──────────────────────■───────┤ H ├─X─
:            │                               │                ┌───┐ │P(π/2) └───┘ │
: q_1: ──────┼────────■──────────────────────┼────────■───────┤ H ├─■─────────X───┼─
...
#+begin_src python
  def make_phase_estimation_circuit(phase_oracle: QuantumCircuit, init_params: str | list | int):
      ...
      qft_dagger = make_qft_circuit(t).inverse()
      qc = qc.compose(qft_dagger, qubits=list(range(t)))
...
#+begin_src python :results replace
  def make_fourier_incrementer_circuit(size: Integral, increment: Integral) -> QuantumCircuit:
      """TODO"""
      qc = QuantumCircuit(size)
  ...
  def make_incrementer_circuit(size: Integral, increment: Integral) -> QuantumCircuit:
      """TODO"""
      qc = QuantumCircuit(size)
      finc = make_fourier_incrementer_circuit(size, increment)
      qft = make_qft_circuit(size)

      qft_gate = qft.to_gate(label="QFT")
      qft_dg_gate = qft.inverse().to_gate(label="QFT^†")
      finc_gate = finc.to_gate(label=f"ϕinc({increment})")

      qc.append(qft_gate, range(size))
      qc.append(finc_gate, range(size))
      qc.append(qft_dg_gate, range(size))

      return qc

  qc1 = make_fourier_incrementer_circuit(4, 3)
  qc2 = make_incrementer_circuit(4, 3)
...
#+begin_src python :results replace
  def make_modular_fourier_incrementer_circuit(increment: int, modulus: int):
      n = modulus.bit_length()
      size = n + 1

      qc_inc_a = make_fourier_incrementer_circuit(increment=increment, size=size)
      qc_inc_N = make_fourier_incrementer_circuit(increment=modulus, size=size)
      qft = make_qft_circuit(size)

      inc_gate_a = qc_inc_a.to_gate(label=f"ϕinc({increment})")
      inc_gate_N = qc_inc_N.to_gate(label=f"ϕinc({modulus})")

      inc_gate_dg_a = qc_inc_a.inverse().to_gate(label=f"ϕinc({increment})^†")
      inc_gate_dg_N = qc_inc_N.inverse().to_gate(label=f"ϕinc({modulus})^†")
      ...
      qc.append(inc_gate_a, qreg)
      qc.append(inc_gate_dg_N, qreg)

      qc.append(qft_dg_gate, qreg)
      qc.cx(qreg[n], qreg_anc[0])
      qc.append(qft_gate, qreg)

      qc.append(inc_gate_N.control(), list(qreg_anc)+list(qreg))  # TODO
      qc.append(inc_gate_dg_a, qreg)

      qc.append(qft_dg_gate, qreg)
      qc.cx(qreg[n], qreg_anc[0], ctrl_state=0)
      qc.append(qft_gate, qreg)
  ...
  def make_modular_incrementer_circuit(increment: int, modulus: int):
      fourier_inc_mod = make_modular_fourier_incrementer_circuit(increment, modulus)
      qreg, qreg_anc = fourier_inc_mod.qregs
      qc = QuantumCircuit(qreg, qreg_anc)

      qft = make_qft_circuit(qreg.size)

      qft_gate = qft.to_gate(label="QFT")
      qft_dg_gate = qft.inverse().to_gate(label="QFT^†")
      fourier_inc_mod_gate = fourier_inc_mod.to_gate(label=f"inc({increment})mod({modulus})")

      qc.append(qft_gate, qreg)
      qc.append(fourier_inc_mod_gate, list(qreg)+list(qreg_anc))
      qc.append(qft_dg_gate, qreg)

      return qc


  qc1 = make_modular_fourier_incrementer_circuit(3, 7)
  qc2 = make_modular_incrementer_circuit(3, 7)
...
q_0: ┤0         ├┤0           ├┤0       ├─────┤0     ├┤0         ├┤0           ├┤0       ├───────────────┤0     ├»
     │          ││            ││        │     │      ││          ││            ││        │               │      │»
q_1: ┤1         ├┤1           ├┤1       ├─────┤1     ├┤1         ├┤1           ├┤1       ├───────────────┤1     ├»
     │  ϕinc(3) ││  ϕinc(7)^† ││  QFT^† │     │  QFT ││  ϕinc(7) ││  ϕinc(3)^† ││  QFT^† │               │  QFT │»
q_2: ┤2         ├┤2           ├┤2       ├─────┤2     ├┤2         ├┤2           ├┤2       ├───────────────┤2     ├»
     │          ││            ││        │     │      ││          ││            ││        │┌───┐     ┌───┐│      │»
q_3: ┤3         ├┤3           ├┤3       ├──■──┤3     ├┤3         ├┤3           ├┤3       ├┤ X ├──■──┤ X ├┤3     ├»
     └──────────┘└────────────┘└────────┘┌─┴─┐└──────┘└────┬─────┘└────────────┘└────────┘└───┘┌─┴─┐└───┘└──────┘»
  a: ────────────────────────────────────┤ X ├─────────────■───────────────────────────────────┤ X ├─────────────»
                                         └───┘                                                 └───┘             »
«     ┌──────────┐
...
#+begin_src python
  def quantum_add(a: int, b: int, modulus: int = None, size: int = None) -> int:
      ...
      if modulus is None:
          ...
          qc = QuantumCircuit(size)
          qc_inc = make_incrementer_circuit(increment=a, size=size)
      else:
          assert a < modulus and b < modulus, "a and b must be smaller than modulus"
          qc_inc = make_modular_incrementer_circuit(increment=a, modulus=modulus)
          size = qc_inc.qregs[0].size + 1  # TODO: ugly
          qc = QuantumCircuit(size)

      qc.initialize(f"{b:0{size}b}")
      qc.compose(qc_inc, inplace=True)  # TODO: ugly
      qc.measure_all()  # TODO: rely on simplification that ancilla is zero
