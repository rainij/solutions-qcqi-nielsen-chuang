...
#+begin_src python
  ...
  from qiskit.circuit.library import TGate, MCPhaseGate
...
** TODO NEW Addition in Fourier Space
#+begin_src python :results replace
  def append_fourier_incrementer(qc: QuantumCircuit, qreg: QuantumRegister | list[int], increment: int):
      """TODO"""
      size = len(qreg)
      for k in range(size):
          qc.p(2*math.pi*increment*2**(-size+k), qreg[k])


  def append_fourier_incrementer_dg(qc: QuantumCircuit, qreg: QuantumRegister | list[int], increment: int):
      """TODO"""
      size = len(qreg)
      for k in range(size):
          qc.p(-2*math.pi*increment*2**(-size+k), qreg[k])
...
#+begin_src python :results replace
  ...
  def make_incrementer(size: Integral, increment: Integral) -> Gate:
      ...
      append_fourier_incrementer(qc, list(range(size)), increment)
...
#+begin_src python :results replace
  ...
  def make_modular_fourier_incrementer(increment: int, modulus: int, num_ctrls: int=0) -> Gate:
      ...
      qc.append(c_inc_N, list(qreg_anc)+list(qreg))  # TODO
...
#+begin_src python :results replace
  def make_VaN(a: int, modulus: int, num_ctrls: int = 0) -> Gate:
      ...
      qreg_ctrl = QuantumRegister(num_ctrls, "c")  # TODO
      ...
      for i in range(int_size - 1):
          ai = a * 2**i % modulus
          cfinc = make_modular_fourier_incrementer(ai, modulus, num_ctrls=1)
...
#+begin_src python
  ...
  def make_order_finding_phase_estimation(a: int, modulus: int, eps: float) -> QuantumCircuit:
      ...
      for i in range(t):
          # TODO: making Ua controlled is slow
          CUai = make_Ua(pow(a, 2**i, modulus), modulus).control()
          qc.append(CUai, [qreg_top[i]] + list(qreg_bot) + list(qreg_anc))
          #Uai = make_Ua(pow(a, 2**i, modulus), modulus)
          #qc.append(Uai, list(qreg_bot) + list(qreg_anc))
