...
*** Solution
Plugging $j=0$ and $N=2^n$ into formula (5.2) we get

$$
  \FT |00\ldots0\rangle = \frac{1}{2 ^{n/2}} \sum_{k=0}^{2^n-1} e^{\frac{2\pi\ii}{2^n}jk} \ket{k} =
...
usual). Note that this corresponds to applying the Hadamard gate to each of the $n$ qubits.
...
/this/ context. We do not need any clever idea here - if I do not overlook something here.
...
<<exercise-5.24-equation-for-s>>
$$
  s = (sl_2) \cdot l_2\inv \mod r .
$$

The inverse of $l_2$ modulo $r$ can be calculated e.g. by euclidean algorithm. But *there
is a catch*. For this to work we need $l_2$ to be invertible modulo $r$. In other words
...
[[exercise-5-22-to-show][formula]] for the Fourier coefficients (and that $f(0,j)=a^j$ is different for each
$j\in\{0,\ldots,r-1\}$).
...
Unfortunately the product on the RHS extended to /all/ primes converges to zero. It equals
$1/\zeta(1)=0$ by the way, where $\zeta$ denotes [[https://en.wikipedia.org/wiki/Riemann_zeta_function][Riemann's zeta]] function. Hence we cannot
...
$r=\gcd(l_2,r)r'$. Instead of the [[exercise-5.24-equation-for-s][original equation]] for $s$ we solve (each time)
...
with $r$ is $1$. In other words, none of the primes contained in $r$ divides /all/ of the
...
the setting of the [[https://en.wikipedia.org/wiki/Chinese_remainder_theorem][Chinese remainder theorem]] which yields $s\mod r$.

Clearly the probability that /not/ all samples are divisible by a particular prime $p_i$
is
...
<<exercise-5-24-lower-bound-Pn>>
$$
  P_n \geq \frac{1}{\zeta(n)} = 1 - O(2^{-n}) .
$$

To see the asymptotic behavior just use the standard formula
$\zeta(n)=\sum_{k=1}^\infty\,k^{-n}$. To see that the product in [[exercise-5-24-pn][the formula]] for $P_n$ is
...
then these random variables are [[https://en.wikipedia.org/wiki/Independence_(probability_theory)][stochastically independent]]. This in turn follows from
...
[[https://en.wikipedia.org/wiki/Conditional_probability][conditional probability]]. This in turn follows from the [[https://en.wikipedia.org/wiki/Chinese_remainder_theorem][Chinese remainder
theorem]]. Especially the formulation in terms of a ring-isomorphism
$\ZZ_{\prod_ip_i}\to\bigoplus_i\ZZ_{p_i}$ makes it apparent.

The [[exercise-5-24-lower-bound-Pn][lower bound]] for $P_n$ shows that a finite number of repetitions suffices to get an
