...
#+begin_src python
  def make_control_prefix(num_ctrls: int):  # TODO: not needed anymore
...
#+begin_src python
  class IncrementGate(Gate):
      ...
      def _define(self):
          ...
          qft = make_qft(self._num_qubits)  # TODO: use builtin?
...
#+begin_src python
  ...
  def make_modular_fourier_incrementer(increment: int, modulus: int, num_ctrls: int=0) -> Gate:
      ...
      return qc.to_gate(label=f"{prefix}ϕinc({increment})mod({modulus})")  # TODO
  ...
  class MCModularFourierIncrementGate(ControlledGate):
      ...
      def _define(self):
          qc = make_modular_fourier_incrementer(
              self._increment, self._modulus, self.num_ctrl_qubits).definition  # TODO
          self.definition = qc
#+end_src
...
#+begin_src python
  def make_UaN(a: int, modulus: int, num_ctrls: int = 0) -> Gate:
      ...
      qreg_ctrl = QuantumRegister(num_ctrls, "c")
      qreg = QuantumRegister(L)
      qreg_anc = AncillaRegister(L + 1)
  ...
  class UaNGate(Gate):
      ...
      def _define(self):
          qc = make_UaN(self._increment, self._modulus).definition  # TODO
          self.definition = qc
  ...
  class MCUaNGate(ControlledGate):
      ...
      def _define(self):
          qc = make_UaN(self._increment, self._modulus, num_ctrls=self.num_ctrl_qubits).definition  # TODO
          self.definition = qc
#+end_src
...
#+RESULTS[7704b501de1eae1176daa94083b565d3ffe6e332]:
#+begin_example
      ┌───────────────────┐            ┌─────────────────────┐
 q_0: ┤0                  ├─X──────────┤0                    ├
      │                   │ │          │                     │
...
#+begin_src python
  ...
  def make_order_finding_phase_estimation(a: int, modulus: int, eps: float) -> QuantumCircuit:
      ...
      for i in range(t):
          # NOTE: There is not big differenc between these two lines wrt performance:
