...
$$
  \ket{j,k,1} \stackrel{W(1,3)}{\mapsto} \ket{j,1,x^j \mod N}
...
there exists an algorithm which uses $O(L^3)$ operations.

Remark: I altered the description a bit since, in my opinion, step two of the original
formulation makes it really hard to find a /clean/ and /simple/ algorithm (if actually
implementing it is the goal). I will explain why in the solution below.

** TODO Exercise 5.18 (Factoring 91)
...
respect to $N$, and show that $x^{r/2} \mod 91 = 64 = − 1 \mod 91$, so the algorithm
succeeds, giving $\gcd(64-1,19)=7$. It is unlikely that this is the most efficient method
