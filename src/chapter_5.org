...
#+begin_src python :results replace
  ...
  def make_incrementer_circuit(size: Integral, increment: Integral) -> QuantumCircuit:
      ...
      qc.compose(finc, inplace=True)
      qc.compose(qft.inverse(), inplace=True)

      return qc

  qc = make_incrementer_circuit(3, 3)
  qc.draw()
#+end_src
...
#+begin_src python
  def make_modular_fourier_incrementer_circuit(increment: int, modulus: int):
      ...
      qc.compose(qc_inc_a, inplace=True)
      qc.compose(qc_inc_N.inverse(), inplace=True)

      qc.compose(qft.inverse(), inplace=True)
      qc.cx(qreg[n], qreg_anc[0])
      qc.compose(qft, inplace=True)

      qc.compose(qc_inc_N, inplace=True)
      qc.compose(qc_inc_a.inverse(), inplace=True)
...
#+begin_src python
  def quantum_add(a: int, b: int, modulus: int = None, size: int = None) -> int:
      ...
      if modulus is None:
          ...
          qc_inc = make_modular_fourier_incrementer_circuit(increment=a, modulus=modulus)
