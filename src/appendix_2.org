...
ground fields then the complex numbers are possible but we do not care here). Homomorphism
just means
...
indecomposable matrix groups are also irreducible. The reverse direction also holds for
finite groups. See [cite:@Fulton2004] Proposition 1.5.
...
preserves the corresponding characters.
...
The definition for dual spaces is motivated by respecting the natural dual pairing

$$ \langle gv^*, gv \rangle_{V^*,V} = \langle v^*, v \rangle_{V^*,V} . $$

The formula for homomorphisms is motivated by $\hom(V,W)=V^*\otimes W$ and is consistent with
$V^*=\hom(V,\KK)$. In fact, the action on $\KK$ is trivial (identity) and hence

\begin{align*}
  \langle v^*, g\inv v\rangle \cdot 1 = \langle (g\inv)^\dagger v^*, v \rangle \\
  g(\langle v^*, \cdot \rangle w) = \langle gv^*, \cdot \rangle gw = \langle v^*, g\inv(\cdot) \rangle gw
\end{align*}

Moreover the dual representation behaves well on Hilbert spaces with the natural identification of
the dual space with the Hilbert space itself: $\rho^*=\rho$ in this case.
...
$S_1$, $S_2$ are two isomorphisms as above then each of them is a multiple of the other
one.
...
- Remarks ::
  ...
  - Some texts denote by $\hom(\rho^V,\rho^W)$ the subset of those homomorphisms between
    $V$ and $W$ ($\hom(V,W)$) which are G-linear. This makes the class of representations
    of $G$ into a [[https://en.wikipedia.org/wiki/Category_(mathematics)][category]]. Schur's Lemma essentially says that for irreducible
    representations $\hom(\rho^V,\rho^W)$ is rather small.
...
- Remarks ::
  ...
  - What is not unique (in general) is the decomposition of the $V_i^{\oplus a_i}$ into $a_i$ copies
    of $V_i$. In fact, look at the trivial group action of $\langle \mathrm{id}\rangle$ on
    $\CC^2$. It decomposes into $V_1^{\oplus2}$ where $V_1$ are trivial too, but on $\CC$. The
    /concrete/ decomposition amounts to a choice of a basis.
...
This holds also for $\hom_G$ replaced by $\hom$. Now suppose that for some $i$ the $V_i$
is not equivalent to any of the $W_j$. Then this together with Schur's Lemma implies that
the identity would map $V_i$ to zero - contradiction. By symmetry this implies that $n=m$
and $V_i=W_{\pi(i)}$ for some permutation $\pi$. Without loss of generality
$\pi=\mathrm{id}$.

Again by Schur's lemma we thus have:
...
Hence the identity on $V$ induces a bijection between $V_i^{a_i}$ and $W_i^{b_i}$. Since the
