...
$$
  (x, y, 1, 0, a, b) \; \mapsto \, (x, y, 1, 0, a\oplus c, b\oplus x\oplus y) ,
$$

where $c=xy$ is the carry-bit. The two bits initially at $1$ and $0$ can be considered
working registers. Note that no garbage is left behind. This operation is implemented by
the following circuit.

#+RESULTS[831d67a54801a5b753343a1f0e06eedd43d6d5ec]: exercise-3.31-circuit
#+begin_example
...
The first Toffoli directly adds the carry-bit to $a$. The second and the third Toffoli
produce $x\oplus y$ within the working area. This is then added to $b$ by the fourth
register. The two remaining Toffolis just uncompute the second and the third Toffoli.
...
#+RESULTS[5a8e86aa717881404a40fc7793fa7d6e379e2dc0]: exercise-3.32-Fred-by-Toff
:
:   c: ──■────■────■──
:        │  ┌─┴─┐  │
:   x: ──■──┤ X ├──■──
...
that the two targets are already fixed to target the $x$ and the $y$ bit. From that it
follows that both Toffolis must have one control at $c$ since $c$ in the Fredkin gate has
an effect on both $x$ and $y$. In the same way the second control is necessarily at $x$ or
$y$ so there is no further freedom.
...
The Toffoli gate can be implemented by three Fredkin gates and two ancilla bits /if/
garbage is allowed:

#+RESULTS[090cc5c8648cb23aa132fd3b8f6aa65427a008b7]: exercise-3.32-Toff-by-Fred
#+begin_example
...
moved into the third wire iff $a$ and $b$ are both $1$.
...
required because the Toffoli gate can either increase or decrease the number of ones.
...
operation must depend on /both/ $a$ and $b$ hence the controls must occupy $a$ and
$b$. Without loss of generality we may assume that the first gate has $a$ as control
and the second gate has $b$ as control. Clearly, the second gate must have the third wire
...
$\overline{x}$ if $a=1$. Clearly there is no way how the first gate could accomplish that
